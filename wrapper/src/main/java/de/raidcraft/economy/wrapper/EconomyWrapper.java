...
@Log(topic = "RCEconomy-Wrapper")
public class EconomyWrapper implements net.milkbowl.vault.economy.Economy, Economy {
    ...
    private static void init() {
        ...
        if (vault == null || !vault.isEnabled()) {
            log.severe("Vault not found! Please install Vault to use this plugin.");
            log.severe("Falling back to empty wrapper implementation...");
            instance = new EmptyWrapper();
            return;
        }

        Plugin rcEconomy = Bukkit.getPluginManager().getPlugin("RCEconomy");
        if (rcEconomy != null && rcEconomy.isEnabled()) {
            instance = new EconomyWrapper(((EconomyPlugin) rcEconomy).getEconomy());
        } else {
            RegisteredServiceProvider<net.milkbowl.vault.economy.Economy> rsp = Bukkit.getServer().getServicesManager()
                    .getRegistration(net.milkbowl.vault.economy.Economy.class);
            if (rsp == null) {
                log.severe("No valid Vault Economy Implementation found!");
                instance = new EmptyWrapper();
                return;
            }
            instance = new EconomyWrapper(rsp.getProvider());
        }
    }
    ...
    public EconomyWrapper(net.milkbowl.vault.economy.Economy economy) {
        this.economy = economy;
    }

    public EconomyWrapper(RCEconomy rcEconomy) {
    ...
    public EconomyResponse depositPlayer(OfflinePlayer player, double amount, String details) {
        ...
        if (isRcEconomyEnabled()) {
            return rcEconomy.depositPlayer(player, amount, details);
        }
